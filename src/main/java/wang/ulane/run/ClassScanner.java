...
public class ClassScanner {
	private static Log LOGGER = LogFactory.getLog(ClassScanner.class);
	...
	public ClassScanner(String[] entityPackage,Class<?> scanClass) {
		...
		// scanClass是注解类型
		if(scanClass != null && scanClass.isAnnotation()) {
	...
	public Set<Class<?>> getClassSet() throws IOException, ClassNotFoundException {
		...
		if (!this.packagesList.isEmpty()) {
			for (String pkg : this.packagesList) {
				List<String> pkgExclude = null;
				if(excludeMapper != null){
					pkgExclude = excludeMapper.get(pkg);
				}
				...
				for (Resource resource : resources) {
					if (resource.isReadable()) {
						...
						if (matchesEntityTypeFilter(reader, readerFactory)) {
							...
							if(pkgExclude != null && pkgExclude.contains(className.substring(className.lastIndexOf(".")+1))){
								if (LOGGER.isDebugEnabled()) {
									LOGGER.debug(className + " now continue...");
									continue;
								}
							}
