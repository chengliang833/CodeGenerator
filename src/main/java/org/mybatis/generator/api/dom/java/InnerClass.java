...
public class InnerClass extends JavaElement {
    ...
    public String getFormattedContent(int indentLevel, CompilationUnit compilationUnit) {
        ...
        if(CodeGeneratorConfig.CUSTOM_FUNC_SELWITHDEATIL.containModel(type.getShortNameWithoutTypeArguments())){
        	OutputUtilities.newLine(sb);
        	Iterator<String> it = CodeGeneratorConfig.CUSTOM_FUNC_SELWITHDEATIL.getSubModel(type.getShortNameWithoutTypeArguments()).iterator();
        	while(it.hasNext()){
        		OutputUtilities.newLine(sb);
        		
        		String subModelName = it.next();
        		Field field2 = new Field(FuncSelWithDeatil.getFirstLower(subModelName), new FullyQualifiedJavaType(subModelName));
        		field2.addJavaDocLine("/**\n     * 子对象\n     */");
        		field2.setVisibility(JavaVisibility.PRIVATE);
        		sb.append(field2.getFormattedContent(indentLevel, compilationUnit));
        		if(it.hasNext()){
        			OutputUtilities.newLine(sb);
        		}
        	}
        ...
        if(CodeGeneratorConfig.CUSTOM_FUNC_SELWITHDEATIL.containModel(type.getShortNameWithoutTypeArguments())){
        	...
        	while(it.hasNext()){
        		...
        		String subModelName = it.next();
        		String firstLowerName = FuncSelWithDeatil.getFirstLower(subModelName);
        		Method method2 = new Method("get"+subModelName);
        		method2.setVisibility(JavaVisibility.PUBLIC);
        		method2.addJavaDocLine("/**\n     * 子对象get\n     */");
        		method2.setReturnType(new FullyQualifiedJavaType(subModelName));
        		method2.addBodyLine("return "+firstLowerName+";");
        		sb.append(method2.getFormattedContent(indentLevel, false, compilationUnit));
        		OutputUtilities.newLine(sb);
        		
        		OutputUtilities.newLine(sb);
        		method2 = new Method("set"+subModelName);
        		method2.setVisibility(JavaVisibility.PUBLIC);
        		method2.addJavaDocLine("/**\n     * 注释注释3\n     */");
        		method2.addParameter(new Parameter(new FullyQualifiedJavaType(subModelName), firstLowerName));
        		method2.addBodyLine("this."+firstLowerName+" = "+firstLowerName+";");
        		sb.append(method2.getFormattedContent(indentLevel, false, compilationUnit));
        		if(it.hasNext()){
        			OutputUtilities.newLine(sb);
        		}
